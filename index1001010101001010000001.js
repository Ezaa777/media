...
// Dipakai untuk memvalidasi :userId dari URL/body SEBELUM dipakai sebagai key
// object atau untuk query data, supaya nilai berbahaya seperti "__proto__",
// "constructor", path traversal (../, /), null byte, dsb langsung ditolak.
// Sengaja dibuat LONGGAR (bukan memaksa format persis dari generator ID saat
// ini) supaya tidak memutus akun lama yang mungkin ID-nya dibuat oleh versi
// kode sebelumnya dengan format sedikit berbeda.
...
function isValidUserId(id) {
  return typeof id === 'string' && SAFE_ID_PATTERN.test(id) && isSafeObjectKey(id);
}
...
const DANGEROUS_EXT_PATTERN = /\.(php[0-9]?|phtml|pht|phar|phps|cgi|pl|py|rb|sh|bash|exe|dll|so|bat|cmd|com|msi|jar|war|jsp|jspx|asp|aspx|cer|asa|asax|ashx|htaccess|htpasswd|ini|conf|config|env|sql)(\.|$)/i;

function hasDangerousFilename(originalname) {
  if (!originalname || typeof originalname !== 'string') return true;
  if (originalname.includes('\0')) return true;
  if (/[\/\\]/.test(originalname)) return true;
  return DANGEROUS_EXT_PATTERN.test(originalname);
}

function bufStartsWith(buf, bytes, offset = 0) {
  if (buf.length < offset + bytes.length) return false;
  for (let i = 0; i < bytes.length; i++) if (buf[offset + i] !== bytes[i]) return false;
  return true;
}
function bufAsciiAt(buf, str, offset) {
  if (buf.length < offset + str.length) return false;
  return buf.toString('ascii', offset, offset + str.length) === str;
}

function detectRealCategory(buf) {
  // IMAGE
  if (bufStartsWith(buf, [0xFF, 0xD8, 0xFF])) return 'image';
  if (bufStartsWith(buf, [0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A])) return 'image';
  if (bufAsciiAt(buf, 'GIF87a', 0) || bufAsciiAt(buf, 'GIF89a', 0)) return 'image';
  if (bufAsciiAt(buf, 'RIFF', 0) && bufAsciiAt(buf, 'WEBP', 8)) return 'image';
  // VIDEO
  if (bufAsciiAt(buf, 'ftyp', 4)) return 'video';
  if (['moov', 'mdat', 'free', 'skip', 'wide'].some(a => bufAsciiAt(buf, a, 4))) return 'video';
  if (bufStartsWith(buf, [0x1A, 0x45, 0xDF, 0xA3])) return 'video';
  if (bufAsciiAt(buf, 'RIFF', 0) && bufAsciiAt(buf, 'AVI ', 8)) return 'video';
  // AUDIO
  if (bufAsciiAt(buf, 'ID3', 0)) return 'audio';
  if (buf[0] === 0xFF && (buf[1] & 0xE0) === 0xE0) return 'audio';
  if (bufAsciiAt(buf, 'RIFF', 0) && bufAsciiAt(buf, 'WAVE', 8)) return 'audio';
  if (bufAsciiAt(buf, 'OggS', 0)) return 'audio';
  if (bufAsciiAt(buf, 'fLaC', 0)) return 'audio';
  return null;
}

const MALICIOUS_SIGNATURES = [
  /<\?php/i, /<\?=/i, /<%@/i, /<%[\s=]/i, /<jsp:/i,
  /<script[\s>]/i, /eval\s*\(\s*(base64_decode|\$_)/i,
  /system\s*\(\s*\$_/i, /passthru\s*\(/i, /shell_exec\s*\(/i, /proc_open\s*\(/i,
  /^#!\s*\/(bin|usr)\//im, /MZ\x90\x00/,
];

function containsMaliciousSignature(buf) {
  const head = buf.subarray(0, Math.min(buf.length, 5 * 1024 * 1024)).toString('latin1');
  const tail = buf.length > 6 * 1024 * 1024
    ? buf.subarray(buf.length - 1024 * 1024).toString('latin1')
    : '';
  return MALICIOUS_SIGNATURES.some(rx => rx.test(head) || (tail && rx.test(tail)));
}

async function deepValidateFile(filePath, expectedCategory) {
  const buf = await fsp.readFile(filePath);
  if (buf.length === 0) throw new Error('File kosong atau rusak');
  const realCategory = detectRealCategory(buf);
  if (!realCategory) throw new Error('Isi file tidak dapat diverifikasi sebagai media yang valid');
  if (realCategory !== expectedCategory) throw new Error('Isi file tidak cocok dengan tipe yang diklaim (file disamarkan)');
  if (containsMaliciousSignature(buf)) throw new Error('File mengandung kode berbahaya dan ditolak');
}

async function reencodeImage(filePath, ext) {
  const formatMap = { '.jpg': 'jpeg', '.jpeg': 'jpeg', '.png': 'png', '.webp': 'webp' };
  const format = formatMap[ext];
  if (!format) return;
  try {
    const output = await sharp(filePath).toFormat(format).toBuffer();
...
function flattenUploadedFiles(reqFiles, reqFile) {
  if (reqFile) return [reqFile];
  if (!reqFiles) return [];
  return Array.isArray(reqFiles) ? reqFiles : Object.values(reqFiles).flat();
}

function expectedCategoryFor(file) {
...
async function verifyUploadedFiles(reqFiles, reqFile) {
  const files = flattenUploadedFiles(reqFiles, reqFile);
  if (files.length === 0) return;
  try {
    for (const file of files) {
      const expected = expectedCategoryFor(file);
      if (!expected) throw new Error('Tipe file tidak dikenal');
      await deepValidateFile(file.path, expected);
      if (expected === 'image') {
        const ext = path.extname(file.filename).toLowerCase();
        await reencodeImage(file.path, ext);
      }
    }
  } catch (err) {
...
// PERBAIKAN KEAMANAN: kalau app ini berjalan di belakang reverse proxy
// (Nginx/Apache/hosting panel), 'trust proxy' wajib diaktifkan supaya
// req.ip (dipakai rate limiter & brute-force tracker) membaca IP asli
// pengunjung, bukan IP proxy itu sendiri — kalau tidak, semua rate limit
// & block-IP jadi tidak berguna karena semua request kelihatan datang
// dari satu IP yang sama.
app.set('trust proxy', 1);

// PERBAIKAN KEAMANAN: middleware blokir IP dipindah ke paling awal (sebelum
// static file serving & semua route). Sebelumnya middleware ini didefinisikan
// di dekat akhir file, sehingga puluhan endpoint — dan bahkan file di
// /uploads serta /public — TIDAK ikut terlindungi ketika sebuah IP diblokir
// oleh admin.
...
// PERBAIKAN KEAMANAN: folder ./data berisi seluruh database JSON (login,
// user, chat, dll) dan TIDAK PERNAH boleh bisa diakses lewat URL apa pun.
// Route ini memblokir secara eksplisit di level aplikasi sebagai lapisan
// tambahan, di luar konfigurasi web-server/hosting panel yang mungkin
// keliru meng-expose-nya (mis. autoindex/file-manager bawaan panel).
app.use(['/data', '/data/*'], (req, res) => res.status(404).json({ error: 'Endpoint tidak ditemukan' }));

// Blokir akses ke dotfile/file konfigurasi (.env, .git, .htaccess, dll) di mana pun.
app.use((req, res, next) => {
  if (/(^|\/)\.(?!well-known)/i.test(req.path)) {
...
// PERBAIKAN KEAMANAN: sebelum file statis dilayani, tolak permintaan yang
// bukan mengarah ke file media yang benar-benar diizinkan. Ini mencegah:
//  - listing folder (mis. GET /uploads/images/)
//  - akses file selain gambar/video/audio yang ekstensinya diizinkan
//    (walaupun secara default express.static tidak melakukan directory
//    listing, ini lapisan pertahanan tambahan kalau ada middleware lain,
//    reverse proxy, atau versi Express berbeda yang perilakunya beda)
app.use('/uploads', (req, res, next) => {
  const cleanPath = req.path.split('?')[0];
  if (cleanPath.endsWith('/') || cleanPath === '') {
    return res.status(403).json({ error: 'Akses folder tidak diizinkan' });
  }
  const ext = path.extname(cleanPath).toLowerCase();
  if (!SAFE_UPLOAD_CONTENT_TYPES[ext]) {
...
app.use('/uploads', express.static('uploads', {
  ...
  setHeaders: (res, filePath) => {
    ...
    const safeType = SAFE_UPLOAD_CONTENT_TYPES[ext] || 'application/octet-stream';
    res.setHeader('Content-Type', safeType);
    res.setHeader('X-Content-Type-Options', 'nosniff');
    res.setHeader('Content-Security-Policy', "default-src 'none'; sandbox");
    if (safeType === 'application/octet-stream') res.setHeader('Content-Disposition', 'attachment');
  }
}));
app.use(express.static('public', { maxAge: '1h', dotfiles: 'deny', index: ['index.html'] }));
...
app.use(session({
  ...
  cookie: {
    ...
    httpOnly: true,
    sameSite: 'lax',
    // PERBAIKAN KEAMANAN: cookie sesi hanya dikirim lewat HTTPS saat
    // berjalan di production, supaya tidak bisa dicuri lewat koneksi HTTP.
...
// ============================================================
// COUPLE FEATURE - Data Structures & Helpers
// ============================================================
async function getCoupleData() {
  const coupleData = await readJSON('data/couple.json');
  return coupleData || { couples: {}, pendingInvites: {}, coupleLevels: {} };
}

async function saveCoupleData(data) {
  return await writeJSON('data/couple.json', data);
}

function getCoupleTitle(level) {
  const titles = {
    1: { name: "💕 Baru Berjumpa", icon: "🌸" },
    2: { name: "💖 Mulai Dekat", icon: "💕" },
    3: { name: "✨ Berdua Manis", icon: "✨" },
    4: { name: "💞 Saling Sayang", icon: "💞" },
    5: { name: "🌹 Romantis", icon: "🌹" },
    6: { name: "⭐ Bintang Berdua", icon: "⭐" },
    7: { name: "💍 Pasangan Sejati", icon: "💍" },
    8: { name: "👑 Couple Royal", icon: "👑" },
    9: { name: "💎 Abadi Selamanya", icon: "💎" },
    10: { name: "🌈 Legendaris", icon: "🌈" },
    11: { name: "🔥 Tak Terpisahkan", icon: "🔥" },
    12: { name: "💫 Bidadari & Kesatria", icon: "💫" },
    13: { name: "🌟 My Everything", icon: "🌟" },
    14: { name: "💝 Soulmate", icon: "💝" },
    15: { name: "🏆 Couple Champion", icon: "🏆" }
  };
  if (level <= 15) return titles[level];
  if (level <= 30) return { name: "⚡ Couple Legend", icon: "⚡" };
  if (level <= 50) return { name: "🐉 Mythic Couple", icon: "🐉" };
  if (level <= 99) return { name: "👁️ Ascended Couple", icon: "👁️" };
  return { name: "✨ Immortal Couple", icon: "✨" };
}

function getCoupleXPNeeded(level) {
  return Math.floor(100 * Math.pow(1.15, level - 1));
}

function calculateCoupleLevel(xp) {
  let level = 1;
  let needed = getCoupleXPNeeded(level);
  while (xp >= needed && level < 100) {
    xp -= needed;
    level++;
    needed = getCoupleXPNeeded(level);
  }
  return { level, xp: Math.floor(xp), nextXP: needed };
}

async function addCoupleXP(coupleId, amount, userId) {
  const coupleData = await getCoupleData();
  if (!coupleData.coupleLevels[coupleId]) {
    coupleData.coupleLevels[coupleId] = { xp: 0, level: 1 };
  }
  
  const couple = coupleData.coupleLevels[coupleId];
  couple.xp += amount;
  
  const levelInfo = calculateCoupleLevel(couple.xp);
  const leveledUp = levelInfo.level > couple.level;
  couple.level = levelInfo.level;
  couple.xp = levelInfo.xp;
  
  await saveCoupleData(coupleData);
  
  if (coupleData.couples[coupleId]) {
    await addXP(coupleData.couples[coupleId].user1, Math.floor(amount / 2));
    await addXP(coupleData.couples[coupleId].user2, Math.floor(amount / 2));
  }
  
  return { leveledUp, newLevel: couple.level, title: getCoupleTitle(couple.level) };
}
...
function initializeDataFiles() {
  const dirs = [
    ...
    './uploads/profiles','./uploads/comments',
    './uploads/wallpapers','./uploads/chat'
  ];
  dirs.forEach(dir => { if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true }); });

  const htaccessContent =
`# Auto-generated oleh sistem — jangan dihapus
...
<FilesMatch "\\.(php[0-9]?|phtml|pht|phar|cgi|pl|py|sh|bash|asp|aspx|jsp|jspx|exe|dll|so)$">
...
`;
  ['./uploads', './uploads/images', './uploads/videos', './uploads/profiles',
   './uploads/comments', './uploads/wallpapers', './uploads/chat']
    .forEach(dir => {
      const f = path.join(dir, '.htaccess');
      if (!fs.existsSync(f)) fs.writeFileSync(f, htaccessContent);
    });
  ...
  const files = {
    ...
    'data/user.json':           {},
    'data/database.json':       { posts: [], comments: [], likes: [], favorites: [] },
    ...
    'data/couple.json':         { couples: {}, pendingInvites: {}, coupleLevels: {} }
  };
...
// FAVORITES HELPERS
// ============================================================
async function getFavorites(userId, currentUserId) {
  const database = await readJSON('data/database.json');
  const favorites = (database.favorites || []).filter(f => f.userId === userId);
  const posts = [];
  for (const fav of favorites) {
    const post = (database.posts || []).find(p => p.id === fav.postId);
    if (post) {
      if (post.isPrivate && post.userId !== currentUserId) continue;
      posts.push({
        postId: post.id,
        mediaUrl: post.mediaUrl,
        mediaType: post.mediaType,
        mediaArray: post.mediaArray,
...
async function toggleFavorite(userId, postId) {
  ...
  const existingIdx = database.favorites.findIndex(f => f.userId === userId && f.postId === postId);
  let action, favoritesCount = 0;
  ...
  if (existingIdx === -1) {
    ...
    action = 'added';
    favoritesCount = database.favorites.filter(f => f.postId === postId).length;
  } else {
    database.favorites.splice(existingIdx, 1);
    action = 'removed';
    favoritesCount = database.favorites.filter(f => f.postId === postId).length;
  }
  
  await writeJSON('data/database.json', database);
  return { success: true, action, favoritesCount };
}

async function isFavorited(userId, postId) {
  const database = await readJSON('data/database.json');
  return !!(database.favorites || []).some(f => f.userId === userId && f.postId === postId);
}

async function getFavoritesCount(postId) {
  const database = await readJSON('data/database.json');
  return (database.favorites || []).filter(f => f.postId === postId).length;
}
...
async function deleteUserData(userId) {
  ...
  database.favorites = (database.favorites || []).filter(f => f.userId !== userId);
  ...
  const coupleData = await getCoupleData();
  let coupleIdToRemove = null;
  for (const [cid, couple] of Object.entries(coupleData.couples || {})) {
    if (couple.user1 === userId || couple.user2 === userId) {
      coupleIdToRemove = cid;
      break;
    }
  }
  if (coupleIdToRemove) {
    delete coupleData.couples[coupleIdToRemove];
    delete coupleData.coupleLevels[coupleIdToRemove];
    await saveCoupleData(coupleData);
  }
...
const storage = multer.diskStorage({
  destination(req, file, cb) {
    ...
    else if (file.mimetype.startsWith('audio/')) folder += 'chat/';
...
const upload = multer({
  ...
  fileFilter(req, file, cb) {
    if (hasDangerousFilename(file.originalname)) return cb(new Error('Nama file tidak diizinkan'));
    const ext = path.extname(file.originalname).toLowerCase();
...
app.post('/api/couple/invite', requireLogin, async (req, res) => {
  const { username } = req.body;
  if (!username) return res.status(400).json({ error: 'Username diperlukan' });
  
  const loginData = await readJSON('data/login.json');
  const targetUser = loginData.users.find(u => u.username.toLowerCase() === username.toLowerCase());
  
  if (!targetUser) return res.status(404).json({ error: 'Pengguna tidak ditemukan' });
  if (targetUser.id === req.session.userId) return res.status(400).json({ error: 'Tidak bisa mengundang diri sendiri' });
  
  const coupleData = await getCoupleData();
  
  for (const [cid, couple] of Object.entries(coupleData.couples || {})) {
    if (couple.user1 === req.session.userId || couple.user2 === req.session.userId) {
      return res.status(400).json({ error: 'Kamu sudah memiliki pasangan' });
    }
    if (couple.user1 === targetUser.id || couple.user2 === targetUser.id) {
      return res.status(400).json({ error: 'Pengguna sudah memiliki pasangan' });
    }
  }
  
  const existingInvite = (coupleData.pendingInvites?.[targetUser.id] || []).find(
    i => i.from === req.session.userId
  );
  if (existingInvite) {
    return res.status(400).json({ error: 'Undangan sudah dikirim sebelumnya' });
  }
  
  if (!coupleData.pendingInvites) coupleData.pendingInvites = {};
  if (!coupleData.pendingInvites[targetUser.id]) coupleData.pendingInvites[targetUser.id] = [];
  
  coupleData.pendingInvites[targetUser.id].push({
  ...
  await saveCoupleData(coupleData);
  res.json({ success: true, message: `Undangan terkirim ke @${username}` });
});

app.get('/api/couple/invites', requireLogin, async (req, res) => {
  const coupleData = await getCoupleData();
  const invites = coupleData.pendingInvites?.[req.session.userId] || [];
  
  const enriched = await Promise.all(invites.map(async inv => {
    const userData = await getUserById(inv.from);
    return {
      ...inv,
      profilePic: userData?.profilePic || '',
      isVerified: await isVerified(inv.from)
    };
  }));
...
app.post('/api/couple/accept', requireLogin, async (req, res) => {
  const { fromUserId } = req.body;
  if (!fromUserId) return res.status(400).json({ error: 'Data tidak lengkap' });
  
  const coupleData = await getCoupleData();
  const invites = coupleData.pendingInvites?.[req.session.userId] || [];
  const invite = invites.find(i => i.from === fromUserId);
  
  if (!invite) return res.status(404).json({ error: 'Undangan tidak ditemukan' });
  
  for (const [cid, couple] of Object.entries(coupleData.couples || {})) {
    if (couple.user1 === req.session.userId || couple.user2 === req.session.userId ||
        couple.user1 === fromUserId || couple.user2 === fromUserId) {
      return res.status(400).json({ error: 'Salah satu pengguna sudah memiliki pasangan' });
    }
  }
  
  const coupleId = 'couple_' + Date.now() + '_' + crypto.randomBytes(8).toString('hex');
  if (!coupleData.couples) coupleData.couples = {};
  
  coupleData.couples[coupleId] = {
    id: coupleId,
    user1: fromUserId,
    user2: req.session.userId,
    username1: invite.fromUsername,
    username2: req.session.username,
    createdAt: new Date().toISOString(),
    totalXP: 0
  };
  
  coupleData.pendingInvites[req.session.userId] = coupleData.pendingInvites[req.session.userId].filter(
    i => i.from !== fromUserId
  );
  
  if (!coupleData.coupleLevels) coupleData.coupleLevels = {};
  coupleData.coupleLevels[coupleId] = { xp: 0, level: 1 };
  
  await saveCoupleData(coupleData);
  
  await addCoupleXP(coupleId, 200, req.session.userId);
  
  res.json({ success: true, coupleId });
});

app.post('/api/couple/reject', requireLogin, async (req, res) => {
  const { fromUserId } = req.body;
  const coupleData = await getCoupleData();
  
  if (coupleData.pendingInvites?.[req.session.userId]) {
    coupleData.pendingInvites[req.session.userId] = coupleData.pendingInvites[req.session.userId].filter(
      i => i.from !== fromUserId
    );
    await saveCoupleData(coupleData);
...
app.get('/api/couple/info', requireLogin, async (req, res) => {
  const coupleData = await getCoupleData();
  let myCouple = null;
  let coupleId = null;
  ...
  if (!myCouple) {
    return res.json({ hasCouple: false });
  }
  
  const levelInfo = coupleData.coupleLevels?.[coupleId] || { xp: 0, level: 1 };
  const levelCalc = calculateCoupleLevel(levelInfo.xp);
  const title = getCoupleTitle(levelCalc.level);
  
  const otherUserId = myCouple.user1 === req.session.userId ? myCouple.user2 : myCouple.user1;
  ...
  res.json({
    ...
    partner: {
      ...
      isVerified: await isVerified(otherUserId)
    },
    level: levelCalc.level,
    xp: levelCalc.xp,
    nextXP: levelCalc.nextXP,
    title: title.name,
    titleIcon: title.icon,
    createdAt: myCouple.createdAt,
    username1: myCouple.username1,
    username2: myCouple.username2
  });
});

app.post('/api/couple/action', requireLogin, async (req, res) => {
  ...
  if (!myCoupleId) return res.status(400).json({ error: 'Kamu belum memiliki pasangan' });
  ...
  const result = await addCoupleXP(myCoupleId, xpGain, req.session.userId);
...
app.get('/api/leaderboard/couple', requireLogin, async (req, res) => {
  const coupleData = await getCoupleData();
  const userData = await readJSON('data/user.json');
  const verifiedData = await readJSON('data/verified.users.json');
  ...
  const leaderboard = [];
  for (const [cid, couple] of Object.entries(coupleData.couples || {})) {
    const levelInfo = coupleData.coupleLevels?.[cid] || { xp: 0, level: 1 };
    const levelCalc = calculateCoupleLevel(levelInfo.xp);
    const title = getCoupleTitle(levelCalc.level);
    
    const user1 = userData[couple.user1];
    const user2 = userData[couple.user2];
    ...
    if (user1 && user2) {
      leaderboard.push({
        ...
        profilePic1: user1.profilePic || '',
        profilePic2: user2.profilePic || '',
        level: levelCalc.level,
        xp: levelInfo.xp,
        title: title.name,
        titleIcon: title.icon,
        isVerified1: verifiedData.verified?.includes(couple.user1) || false,
        isVerified2: verifiedData.verified?.includes(couple.user2) || false,
  ...
  for (let i = 0; i < leaderboard.length; i++) {
    const couple = coupleData.couples[leaderboard[i].coupleId];
    if (couple && (couple.user1 === req.session.userId || couple.user2 === req.session.userId)) {
      myRank = i + 1;
      myCouple = leaderboard[i];
      break;
    }
  }
  ...
  res.json({
    success: true,
    topCouples: leaderboard.slice(0, 50),
    myRank: myRank !== -1 ? myRank : null,
    myCouple,
    totalCouples: leaderboard.length
  });
});

app.post('/api/couple/breakup', requireLogin, async (req, res) => {
  const coupleData = await getCoupleData();
  ...
  if (!coupleIdToRemove) {
    return res.status(400).json({ error: 'Kamu tidak memiliki pasangan' });
  }
  ...
  await saveCoupleData(coupleData);
  res.json({ success: true, message: 'Kamu telah berpisah 😢' });
});
...
app.post('/api/register',
  ...
  async (req, res) => {
    ...
    userData[userId] = {
      ...
      links: []  // <--- TAMBAHAN: field links untuk menyimpan link profil
    };
    const levelsData = await readJSON('data/levels.json');
    levelsData.userLevels[userId] = { xp: 0, level: 1, totalPosts: 0, totalLikes: 0 };
...
app.get('/api/me', requireLogin, async (req, res) => {
  ...
  res.json({
    ...
    links: user.links || []  // <--- TAMBAHAN
  });
});

app.get('/api/profile/:userId', requireLogin, async (req, res) => {
  const targetId = sanitizeText(req.params.userId, 50);
  if (!isValidUserId(targetId)) return res.status(400).json({ error: 'User ID tidak valid' });
...
app.post('/api/profile/update', requireLogin, async (req, res) => {
  ...
  const links    = req.body.links || [];  // array of { label, url }
  
  // Validasi links: maksimal 4, setiap link harus punya label dan url valid
  if (!Array.isArray(links)) {
    return res.status(400).json({ error: 'links harus berupa array' });
  }
  if (links.length > 4) {
    return res.status(400).json({ error: 'Maksimal 4 link' });
  }
  for (const link of links) {
    if (!link.label || typeof link.label !== 'string' || link.label.length === 0 || link.label.length > 30) {
      return res.status(400).json({ error: 'Label link harus diisi (1-30 karakter)' });
    }
    if (!link.url || typeof link.url !== 'string' || !isValidUrl(link.url)) {
      return res.status(400).json({ error: `URL tidak valid: ${link.url}` });
    }
  }
  ...
  userData[req.session.userId].links = links;
...
app.post('/api/profile/upload-pic', requireLogin, (req, res) => {
  const uploadMw = profileUpload.fields([{ name:'profilePic', maxCount:1 }, { name:'wallpaper', maxCount:1 }]);
  uploadMw(req, res, async (err) => {
    if (err) return res.status(400).json({ error: err.message });
    try {
      await verifyUploadedFiles(req.files, null);
    } catch (verr) {
...
app.post('/api/profile/upload-wallpaper', requireLogin, (req, res) => {
  const uploadMw = wallpaperUpload.single('wallpaper');
  uploadMw(req, res, async (err) => {
    if (err) return res.status(400).json({ error: err.message });
    if (!req.file) return res.status(400).json({ error: 'Tidak ada file' });
    try {
      await verifyUploadedFiles(null, req.file);
    } catch (verr) {
      return res.status(400).json({ error: verr.message });
    }
    try {
...
app.post('/api/upload',
  ...
  (req, res) => {
    const uploadMiddleware = upload.fields([
      ...
      { name:'media3', maxCount:1 }, { name:'media4', maxCount:1 }
    ]);
    uploadMiddleware(req, res, async (err) => {
      ...
      try {
        ...
        try {
          await verifyUploadedFiles(files, null);
        } catch (verr) {
          return res.status(400).json({ error: verr.message });
        }
        const mediaFiles = [];
        ...
        const newPost = {
          ...
          caption, likes: [], comments: [], isPrivate,
          createdAt: new Date().toISOString()
        };
...
app.get('/api/favorites', requireLogin, async (req, res) => {
  const favorites = await getFavorites(req.session.userId, req.session.userId);
  res.json({ favorites });
});

app.post('/api/favorite/toggle', requireLogin, async (req, res) => {
  const { postId } = req.body;
  if (!postId) return res.status(400).json({ error: 'Post ID diperlukan' });

  const database = await readJSON('data/database.json');
  const post = (database.posts || []).find(p => p.id === postId);
  if (!post) return res.status(404).json({ error: 'Post not found' });

  if (post.isPrivate && post.userId !== req.session.userId) {
    return res.status(403).json({ error: 'Postingan private tidak dapat difavoritkan' });
  }

  const result = await toggleFavorite(req.session.userId, postId);
  res.json(result);
});

app.get('/api/user/:userId/favorites', requireLogin, async (req, res) => {
  const targetId = sanitizeText(req.params.userId, 50);
  if (!isValidUserId(targetId)) return res.status(400).json({ error: 'User ID tidak valid' });
  const favorites = await getFavorites(targetId, req.session.userId);
  res.json({ favorites });
});

// COMMENTS
app.post('/api/comment/upload-media', requireLogin, (req, res) => {
  const uploadMw = commentUpload.single('media');
  uploadMw(req, res, async (err) => {
    if (err) return res.status(400).json({ error: err.message });
    if (!req.file) return res.status(400).json({ error: 'Tidak ada file' });
    try {
...
app.get('/api/post/:postId/comments', requireLogin, async (req, res) => {
  ...
  const post     = (database.posts || []).find(p => p.id === postId);
  if (!post) return res.status(404).json({ error: 'Post not found' });
  // PERBAIKAN KEAMANAN: cegah komentar dari post privat orang lain terbaca
  // hanya dengan menebak/mengetahui postId.
  if (post.isPrivate && post.userId !== req.session.userId) {
    return res.status(403).json({ error: 'Postingan ini privat' });
  }
...
app.get('/api/feed', requireLogin, async (req, res) => {
  ...
  const posts = await Promise.all((database.posts || [])
    // PERBAIKAN KEAMANAN: sebelumnya SEMUA post (termasuk isPrivate:true milik
    // user lain) ikut dikembalikan di sini. Sekarang post privat hanya boleh
    // muncul untuk pemiliknya sendiri.
    .filter(post => !post.isPrivate || post.userId === req.session.userId)
    .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
    .map(async post => ({
      ...post,
      likesCount:      post.likes?.length || 0,
      commentsCount:   post.comments?.length || 0,
      favoritesCount:  await getFavoritesCount(post.id),
      isLiked:         post.likes?.includes(req.session.userId) || false,
      isFavorited:     await isFavorited(req.session.userId, post.id),
...
app.get('/api/user/:userId/posts', requireLogin, async (req, res) => {
  ...
  const posts = (database.posts || [])
    // PERBAIKAN KEAMANAN: sebelumnya endpoint ini mengembalikan SEMUA post
    // milik targetId tanpa mengecek isPrivate, sehingga siapa pun yang login
    // bisa membaca post privat orang lain hanya dengan mengetahui userId-nya
    // (misalnya dari /api/profile/:userId). Sekarang post privat hanya
    // ditampilkan jika yang meminta adalah pemilik post itu sendiri.
    .filter(p => p.userId === targetId && (!p.isPrivate || targetId === req.session.userId))
    .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
...
app.post('/api/post/:postId/like', requireLogin,
  ...
  async (req, res) => {
    ...
    if (post.isPrivate && post.userId !== req.session.userId) {
      return res.status(403).json({ error: 'Postingan private tidak dapat disukai' });
    }
...
app.get('/api/explore', requireLogin, async (req, res) => {
  ...
  const posts = await Promise.all((database.posts || [])
    .filter(p => !p.isPrivate || p.userId === req.session.userId)
    .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
    ...
    .map(async p => ({
      ...
      favoritesCount: await getFavoritesCount(p.id),
      isLiked:        p.likes?.includes(req.session.userId) || false,
      isFavorited:    await isFavorited(req.session.userId, p.id),
      isVerified:     verifiedData.verified?.includes(p.userId) || false,
      userProfilePic: userData[p.userId]?.profilePic || ''
    })));
...
app.post('/api/chat/upload-media', requireLogin, (req, res) => {
  const uploadMw = chatMediaUpload.single('media');
  uploadMw(req, res, async (err) => {
    if (err) return res.status(400).json({ error: err.message });
    if (!req.file) return res.status(400).json({ error: 'Tidak ada file' });
    try {
...
app.post('/api/admin/make-admin/:userId', requireAdmin, async (req, res) => {
  const targetId  = sanitizeText(req.params.userId, 50);
  if (!isValidUserId(targetId)) return res.status(400).json({ error: 'User ID tidak valid' });
  if (!await getUserById(targetId)) return res.status(404).json({ error: 'User tidak ditemukan' });
  const loginData = await readJSON('data/login.json');
  if (!loginData.adminUsers) loginData.adminUsers = [];
...
app.delete('/api/admin/post/:postId', requireAdmin, async (req, res) => {
  ...
  if (database.favorites) database.favorites = database.favorites.filter(f => f.postId !== postId);
  const userData = await readJSON('data/user.json');
...
app.post('/api/admin/assign-badge', requireAdmin, async (req, res) => {
  ...
  if (!isValidUserId(userId)) return res.status(400).json({ error: 'User ID tidak valid' });
...
app.listen(PORT, () => {
  console.log(`✅ Mio Media (FULL FEATURES + COUPLE + LINKS) berjalan di http://localhost:${PORT}`);
  console.log(`🚀 Fitur: Privasi Postingan + Favorite Postingan + COUPLE SYSTEM + LINK PROFIL`);
  console.log(`💕 Fitur Couple: Undangan, Level, Gelar, Leaderboard, Aksi Berdua`);
  console.log(`🔗 Fitur Link: Maksimal 4 link (label + URL) di profil`);
  console.log(`📁 Data tersimpan di folder ./data`);
});
